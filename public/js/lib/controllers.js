pollsApp.controller('IndexController', function ($scope, pollService) {
    ...
    $scope.loadPolls = function () {
        pollService.getData('polls', pageNumber).then(function (response) {
...
pollsApp.controller('PollController', function ($scope, $routeParams, pollService) {
    ...
    pollService.getData('poll', $routeParams.id).then(function (response) {
            var data = response.data;
    ...
    $scope.save = function () {
        if ($scope.poll.option) {
            pollService.postData($routeParams.id, {option: $scope.poll.option}).then(function (response) {
                    var data = response.data;
...
pollsApp.controller('StatsController', function ($scope, $routeParams, pollService) {
    ...
    pollService.getData('stats', $routeParams.id).then(function (response) {
            var data = response.data;
